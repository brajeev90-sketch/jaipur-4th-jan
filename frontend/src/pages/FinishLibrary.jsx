...
import { finishApi, templatesApi } from '../lib/api';
...
export default function FinishLibrary() {
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
      const response = await finishApi.uploadExcel(file);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingItem) {
        await finishApi.update(editingItem.id, { ...formData, id: editingItem.id, created_at: editingItem.created_at });
  ...
  return (
    <div className="animate-fade-in" data-testid="finish-library-page">
      ...
      <div className="page-header flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
        <div>
          <h1 className="page-title" data-testid="finish-title">{t('finishLibrary')}</h1>
          <p className="page-description">{t('finishLibraryDesc')}</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <Button className="gap-2 flex-1 sm:flex-none" onClick={() => openDialog()} data-testid="add-finish-btn">
            <Plus size={18} />
            {t('addFinish')}
          </Button>
        </div>
      ...
      {items.length === 0 ? (
        <Card><CardContent className="py-12 text-center">
          <p className="text-muted-foreground mb-4">{t('noFinishYet')}</p>
          <Button variant="outline" onClick={() => openDialog()}>{t('addFirstFinish')}</Button>
        </CardContent></Card>
      ) : (
          ...
          {items.map((item) => (
            <Card key={item.id} className="card-hover overflow-hidden">
              {item.image ? (
                ...
                <div className="h-40 flex items-center justify-center" style={{ backgroundColor: item.color || '#d4a574' }}>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle className="font-serif text-xl flex items-center gap-2"><FileSpreadsheet size={24} />{t('bulkUploadFinish')}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-4">
            {/* Download Sample */}
            <div className="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <a href={templatesApi.finishSample()} download>
